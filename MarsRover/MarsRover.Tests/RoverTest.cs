...
namespace MarsRover.Tests
{
    public class Tests
    {
        ...
        public void Setup()
        {
            rover = new Rover(new Coordinate(1, 2, CardinalPoint.N));
        }
        ...
        public void MoveRoverWithValidInstructions_ShouldUpdatePositionAndReturnDistance()
        {
            ...
            rover = new Rover(new Coordinate(3, 3, CardinalPoint.E));
            rover.MoveRover("MMRMMRMRRM").Should().Be(Math.Round(Math.Sqrt(8),2));
            rover.GetCurrentPosition().Should().Be("5 1 E");
        }

        [Test]
        public void MoveRoverWithInvalidInstructions_ShouldUpdatePositionAndReturnDistance()
        {
            rover = new Rover(new Coordinate(1, 2, CardinalPoint.N));
            rover.MoveRover("LMLMLMLMM").Should().Be(1);
            rover.GetCurrentPosition().Should().Be("1 3 N");
        ...
        {
            rover = new Rover(new Coordinate(1, 3, CardinalPoint.N));
            rover.MoveRover("RMLMMMM").Should().Be(-1);

            rover = new Rover(new Coordinate(4, 5, CardinalPoint.W));
            rover.MoveRover("RRMRMMMMM").Should().NotBe(-1);

            rover = new Rover(new Coordinate(2, 1, CardinalPoint.S));
